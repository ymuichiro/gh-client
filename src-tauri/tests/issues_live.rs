...
#[test]
fn live_list_issues_against_real_gh() {
    ...
    let service = IssuesService::new(registry, executor);

    let trace = TraceContext::new("live-test-issues-list");
    let result = service.list(&owner, &repo, 20, &trace);

    assert!(
        result.is_ok(),
        "live issue list should succeed: {:?}",
        result.err()
    );
}
