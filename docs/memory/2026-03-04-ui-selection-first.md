...
- フロントエンドの top context（owner/repo）を手入力から選択式へ変更。
- コマンドフォームの `owner/repo` を選択式へ変更し、可能な項目（branch/tag/PR番号/Issue番号/run_id）を動的候補選択化。
- E2E を select ベースの context 設定へ更新。
